"""분석 라우터 — 사정률 빈도/구간/갭/종합/결합/상관/회귀/추세/백테스트/
클러스터링/유사 공고/Walk-forward/Sliding-review/연도별 검증/엑셀 export/
추첨예가 빈도 등 16개 분석 엔드포인트.

server.py 의 `/api/v1/analysis/*` 핸들러를 APIRouter 로 이전한다 (F4).
"""
...
from app.services.history import save_query_history
from app.services.sync import get_announcement_url

router = APIRouter(prefix="/api/v1/analysis", tags=["분석"])
...
@router.get("/frequency/{announcement_id}")
...
@router.get("/rate-buckets/{announcement_id}")
@_rate_limit("60/minute")
...
@router.get("/company-rates/{announcement_id}")
@_rate_limit("60/minute")
...
@router.get("/comprehensive/{announcement_id}")
@_rate_limit("60/minute")
...
@router.get("/combined-prediction/{announcement_id}")
...
@router.get("/correlation/{announcement_id}")
...
@router.get("/regression/{announcement_id}")
@_rate_limit("30/minute")
...
@router.get("/trend/{announcement_id}")
@_rate_limit("60/minute")
def analysis_trend(
    request: Request,
    announcement_id: str,
...
@router.get("/backtest/{announcement_id}")
@_rate_limit("10/minute")
...
@router.get("/similar/{announcement_id}")
@_rate_limit("30/minute")
...
@router.get("/walk-forward/{announcement_id}")
@_rate_limit("10/minute")
...
@router.get("/sliding-review/{announcement_id}")
@_rate_limit("30/minute")
...
@router.get("/yearly-validation/{announcement_id}")
@_rate_limit("30/minute")
...
@router.get("/preliminary-frequency/{announcement_id}")
