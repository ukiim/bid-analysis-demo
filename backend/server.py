...
def _run_sync_for_source(source: str, trigger: str = "manual",
    ...
    Item 1 — 진행률 + chunk-level resume 지원:
      ...
      - resume_from_log_id 가 있고 last_page>0 이면 그 페이지부터 재개.
    """
    db = SessionLocal()
    ...
    def _count_windows(total_days: int, win_days: int, start_date: "str | None") -> int:
        try:
            return sum(1 for _ in _windows_iter(total_days, win_days, start_date=start_date))
        except Exception:
            return 1
    try:
        if api_key:
...
@app.get("/api/v1/admin/sync/progress")
...
def retry_sync(sync_id: str, current_user: User = Depends(require_admin)):
    ...
    Item 1 — last_page>0 이면 해당 페이지 다음부터 chunk-level resume.
    """
    db = SessionLocal()
    resume_id = None
    try:
